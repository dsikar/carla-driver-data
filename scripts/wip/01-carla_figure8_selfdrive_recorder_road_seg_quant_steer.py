...
def compute_control(vehicle, target_wp, target_speed):
    ...
    raw_steering = math.atan2(right_dot, forward_dot) / math.pi

    # HYPER-TWITCHY QUANTIZATION
    def hyper_twitchy_steering(value):
        # 1. Extreme amplification (5x)
        amplified = value * 5.0
        
        # 2. Ultra-fine steps (0.02 increments)
        steering_levels = [round(x*0.02, 3) for x in range(-32, 33)]  # -0.64 to +0.64
        
        # 3. Only pure 0.0 gets zero (no deadzone)
        if amplified == 0.0:
            return 0.0
            
        # 4. Add random noise (±0.03) for artificial jitter
        noise = (random.random() - 0.5) * 0.06  # ±0.03
        noised = amplified + noise
        
        # 5. Hard clamp and quantize
        clamped = max(-0.65, min(0.65, noised))
        return min(steering_levels, key=lambda x: abs(x - clamped))

    control.steer = hyper_twitchy_steering(raw_steering)

    # Debug output shows the chaos
    print(f"Steering - Raw: {raw_steering:.4f} → Amplified: {raw_steering*5.0:.4f} → Final: {control.steer:.4f}")

    # (Keep original throttle/brake logic)
...
if __name__ == "__main__":
    ...
    main('config_640x480_laneid_1_quant.json')
