...
class CarlaDataset(Dataset):
    ...
    def prepare_image_for_neural_network(self, image_path: str) -> np.ndarray:
        ...
        plt.title("YUV converted back to RGB")
        plt.savefig('processed_image.png')
        plt.show()
        
        # Stop execution
        import sys
        print(f"Processed image from: {image_path}")
        sys.exit()
...
config = load_config('config_640x480_segmented.json')
segdir = config['output']['directory']
# Get file pairs
data_dir = f"/home/daniel/git/carla-driver-data/scripts/wip/{segdir}/"  # adjust path as needed
...
# Create datasets
crop_top = config['image_processing']['crop_top']
crop_bottom = config['image_processing']['crop_bottom']
train_dataset = CarlaDataset(train_pairs, crop_top=crop_top, crop_bottom=crop_bottom)
val_dataset = CarlaDataset(val_pairs, crop_top=crop_top, crop_bottom=crop_bottom)
# train_dataset = CarlaDataset(train_pairs, crop_top=220, crop_bottom=480)
# val_dataset = CarlaDataset(val_pairs, crop_top=220, crop_bottom=480)
...
# Add these lines to visualize the first image
# import matplotlib.pyplot as plt

# # Get the first image from the batch and convert for visualization
# first_image = images[0].permute(1, 2, 0).numpy()  # Convert from CHW to HWC format
# first_image_rgb = cv2.cvtColor(first_image, cv2.COLOR_YUV2RGB)  # Convert YUV to RGB

# # Plot the image
# plt.figure(figsize=(8, 6))
# plt.imshow(first_image_rgb)
# plt.title(f"First Image (Steering angle: {steering[0].item():.4f})")
# plt.savefig('first_processed_image.png')
# plt.show()

# # Stop execution
# import sys
# print("Execution stopped after plotting the first image.")
# sys.exit()
# END DEBUG IMAGE #
...
model_save_path = f'best_segmentation_steering_model_{timestamp}.pth'
