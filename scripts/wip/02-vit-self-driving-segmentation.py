...
class CarlaSteering:
    ...
    def setup_vehicle_and_cameras(self):
        ...
        rgb_bp.set_attribute('image_size_x', str(IMAGE_SIZE[1]))
        rgb_bp.set_attribute('image_size_y', str(IMAGE_SIZE[0]))
        ...
        seg_bp.set_attribute('image_size_x', str(IMAGE_SIZE[1]))
        seg_bp.set_attribute('image_size_y', str(IMAGE_SIZE[0]))
    ...
    def process_rgb_image(self, image):
        rgb_array = np.frombuffer(image.raw_data, dtype=np.uint8).reshape(IMAGE_SIZE[0], IMAGE_SIZE[1], 4)[:, :, :3]
        self.latest_rgb = rgb_array

    def process_seg_image(self, image):
        seg_array = np.frombuffer(image.raw_data, dtype=np.uint8).reshape(IMAGE_SIZE[0], IMAGE_SIZE[1], 4)[:, :, :3]
    ...
    def process_combined_image(self):
        ...
        overlaid_image[lane_mask == 255] = [0, 255, 0]
        ...
        with open(PREDICTION_FILE, "r") as f:
            ...
            if lines:
                try:
                    ...
                    PREDICTION_FILE.unlink()  # Explicitly delete prediction file
        ...
        try:
            ...
            while True:
                ...
                if combined_img is not None:
                    ...
                    print(f"Generated {image_filename} with lane segmentation")
                    
                    # Send for prediction only if not awaiting one
